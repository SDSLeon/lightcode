...
export function FloatingComposerDock(props: {
  ...
  // Reset during render so every collapse opens its single 0.2s envelope with
  // full content before the timer below flips to compact metrics mid-fade.
  // The timer itself stays in the effect (a side effect); only the synchronous
  // reset moves out of it.
  const [prevCollapsing, setPrevCollapsing] = useState(collapsing);
  if (prevCollapsing !== collapsing) {
    setPrevCollapsing(collapsing);
